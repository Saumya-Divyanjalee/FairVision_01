...
# CNN Model — only define if torch available
if TORCH_OK:
    ...
    class FairVisionCNN(nn.Module):
        def __init__(self, num_classes=9):
            ...
            self.classifier = nn.Sequential(
                ...
                nn.Dropout(0.3), nn.Linear(1024,512), nn.ReLU(inplace=True),
...
if uploaded_file:
    ...
    with right_col:
        ...
        for i,(label,prob) in enumerate(results):
            ...
            <div class="pred-card {RANK_CLASSES[i]}">
                <div style="display:flex;justify-content:space-between;align-items:center;">
                    <span style="color:white;font-size:1rem;">{RANK_LABELS[i]} &nbsp; Age Group: <strong style="color:{clr}">{label}</strong></span>
                    <span style="color:{clr};font-size:1.2rem;font-weight:bold;">{pct:.1f}%</span>
                </div>
        ...
        st.markdown(f"""
        <div class="result-highlight" style="background:{top_clr}18;border:1px solid {top_clr};">
    ...
    with st.expander("View Raw Prediction Data"):
        raw_df = pd.DataFrame({"Age Group":AGE_NAMES,"Confidence":[f"{p*100:.2f}%" for p in all_probs],"Raw Score":[f"{p:.6f}" for p in all_probs]})
    ...
    <div class="upload-section">
        ...
        <p>Supported formats: JPG, JPEG, PNG<br>Use a clear front-facing face photo for best results.</p>
    </div>""", unsafe_allow_html=True)
...
| Block | Input | Output | Spatial Size |
|-------|-------|--------|--------------|
...
Optimizer: Adam (lr=0.001) | Loss: CrossEntropyLoss with class weights | Scheduler: StepLR
    """)
...
**Audit Groups:** Race (7 groups) and Gender (Male/Female)

**M1 — Class-Weighted Loss (Active):** Inverse-frequency weights force model to learn rare age classes (0-2, 70+) better.

**M2 — Balanced Mini-Batches:** WeightedRandomSampler ensures balanced age class representation per batch.
    """)
...
**NOT suitable for:** Legal age verification, medical decisions, surveillance, high-stakes decisions.

**Acceptable for:** Research, education, analytics with human oversight.

**Conditions:** Human review required | Regular bias re-audits | Transparent disclosure of performance gaps.
    """)
...
<div class="footer">
    ...
    <span style="color:#1f77b4;">Dataset: FairFace | Framework: PyTorch | UI: Streamlit</span>
</div>""", unsafe_allow_html=True)